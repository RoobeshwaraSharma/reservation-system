...
import { ServicesType } from "@/lib/quaries/getServices";

type Props = {
  data: ServicesType;
};

type RowType = ServicesType[0];
  ...
  const columns = columnHeadersArray.map((columnName) =>
    columnHelper.accessor(
      (row) => {
        ...
        if (
          columnName === "chargePerPerson" &&
          value !== null &&
          typeof value === "string"
        ) {
          // Convert chargePerPerson string to number and format as currency
          ...
          <TableBody>
            {table.getRowModel().rows.map((row) => (
              <TableRow
                ...
                onClick={() =>
                  router.push(`/services/form?serviceId=${row.original.id}`)
                }
              >
